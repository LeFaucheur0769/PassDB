...
def searcher(config, outputFile):
    ...
    if config.get("debug") == True:
        print("outputFile inside searcher : ", outputFile)
    if config.get("debug") == True:
        print("outputFile type : ", type(outputFile))
    tui.printLogo()
    ...
    print(
        "Exporting to: ",
        os.path.join(str(config.get("export_results_location")), str(outputFile)),
    )
    ...
    for filename in os.listdir(config.get("db_location") + "/sorted"):
        ...
        if filename.endswith(".txt"):
            ...
            if (
                str(filename)
                == str(firstletter) + str(secondletter) + str(thirdletter) + ".txt"
                or str(filename)
                == str(firstletter).upper()
                + str(secondletter).upper()
                + str(thirdletter).upper()
                + ".txt"
            ):
                if config.get("debug"):
                ...
                for line in filecontent:
                    if email in line:
                        ...
                        if config.get("debug") == True:
                            print(
                                config.get("export_results_location")
                                + " export location"
                            )
                        if config.get("debug") == True:
                            print(
                                os.path.join(
                                    str(config.get("export_results_location")),
                                    outputFile,
                                )
                                + " os.path.join"
                            )
                        if outputFile == None:
                            ...
                            with open(
                                ...
                                "a+",
                                encoding="utf-8",
                            ) as outputFile_object:
                                outputFile_object.write(line)
                                outputFile_object.close()
                                if config.get("print_result_export_file") == True:
                                    print(line)
                    else:
                        # print(line, "!=", email)
                        None
