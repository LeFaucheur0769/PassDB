...
#[derive(Debug)]
enum WorkerProgress {
    FileProgress(usize, f64),
    FileCompleted(usize, String, bool),
    SortCompleted(usize, color_eyre::Result<String>),
    WorkerFinished,
}
...
impl AddDbScreen {
    ...
    fn start_worker(&mut self, db_location: String) {
        ...
        thread::spawn(move || {
            process_files_worker(files_clone, db_location, progress_tx, command_rx, log_tx);
        });
            ...
            loop {
                match rx.try_recv() {
                    ...
                    Ok(WorkerProgress::FileCompleted(file_index, hash, file_exist)) => {
                        ...
                        if file_index < self.files.len() {
                            ...
                            if !file_exist {
                                ...
                                self.logs.push(LogEntry::error("File has already been imported"));
                            }
                        }
                        ...
                        // Move to next unprocessed file
                        self.move_to_next_file(file_index);
                    }
                    Ok(WorkerProgress::SortCompleted(file_index, result)) => {
                        match result {
                            Ok(message) => {
                                if file_index < self.files.len() {
                                    self.logs.push(LogEntry::success(format!(
                    ...
                    Ok(WorkerProgress::WorkerFinished) => {
                        ...
                        self.logs.push(LogEntry::success("Worker thread finished"));
                        self.progress_total = 1.0;
                        self.logs.push(LogEntry::success("=== PROCESSING COMPLETE ==="));
                        self.logs.push(LogEntry::info("Press 'q' to go back"));
                    }
                    Err(TryRecvError::Empty) => break,
                    Err(TryRecvError::Disconnected) => {
                        self.is_processing = false;
                        self.logs.push(LogEntry::error("Worker thread disconnected"));
                        break;
                    }
                }
            }
...
impl Screen for AddDbScreen {
        ...
        match key.code {
            KeyCode::Char('q') | KeyCode::Esc => {
                if self.is_processing {
                    ...
                    self.logs.push(LogEntry::info("Stopping worker and exiting..."));
            ...
            KeyCode::Char('s') => {
                ...
                self.logs.push(LogEntry::info("Skipping current file"));

                // Mark current as processed and move to next
    ...
    fn draw(&mut self, frame: &mut Frame) {
        ...
        let logs_render = List::new(logs_vec)
...
// Worker thread function (unchanged from your original)
            ...
            while !file_processed && !should_stop && !skip_current {
                ...
                if hashfile.update().unwrap_or(false) {
                    ...
                    if let Ok((hash, file_exist)) = hashfile.finalize() {
                        ...
                        // If the file is new, sort it into the database
                        if !file_exist {
                            match sorter::Sort::new(file_path, &db_location) {
                                Ok(mut sorter) => {
                                    let result = sorter.sort_into_db(&conn, &log_tx);
                                    let _ = progress_tx.send(WorkerProgress::SortCompleted(file_index, result));
                                }
                                Err(e) => {
